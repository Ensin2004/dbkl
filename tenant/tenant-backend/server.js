...
app.get("/tenant/IC/:IC", (req, res) => {
  ...
  connection.query(sql, [IC], (err, results) => {
    ...
    // check ic exist or not, if no exist
    if (results.length === 0) {
      return res.status(404).json({ message: "IC number does not exist" });
    }

    // ic exist
    const { status } = results[0];
    res.status(200).json({ message: "IC exists", status });
  });
});
...
app.get("/tenant/fetch-image/:ic", (req, res) => {
  ...
  connection.query(sql, [ic], (err, results) => {
    ...
    // Log query results
    console.log(
      `Query executed for IC: ${ic}. Results: ${JSON.stringify(results)}`
    );
    ...
    if (results.length > 0 && results[0].image_url) {
      ...
      const imagePath = path.join(
        __dirname,
        "../tenant-backend/assets/tenantImg",
        results[0].image_url
      );
